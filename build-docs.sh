...
# Derive _data/repos.yml from the manifest, so the "View on GitHub" links on
# every app page follow a repo rename instead of rotting. Manifest URLs are SSH
# (git@github.com:Org/Repo.git); readers need the browser form.
#
# The Exchange links this sits beside are hand-maintained in _data/exchange.yml
# -- a listing id appears nowhere in the repo, so there is nothing to derive.
write_repo_data() {
  local manifest
  for candidate in manifest aio/manifest ../manifest; do
    [ -f "$candidate" ] && manifest="$candidate" && break
  done
  if [ -z "$manifest" ]; then
    echo "Warning: no manifest found, skipping repo links" >&2
    return 0
  fi

  mkdir -p _data
  {
    echo "# Generated by build-docs.sh from $manifest -- do not edit."
    echo "# Repo links follow the manifest, so a rename cannot leave a dead link."
    while read -r repo dir _branch; do
      [[ -z "$repo" || "$repo" =~ ^# ]] && continue
      # git@github.com:Org/Repo.git  ->  https://github.com/Org/Repo
      local url="${repo#git@github.com:}"
      url="https://github.com/${url%.git}"
      echo "$dir: $url"
    done < "$manifest"
  } > _data/repos.yml
  echo "Wrote _data/repos.yml from $manifest"
}
